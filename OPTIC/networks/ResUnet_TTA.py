...
class ResUnet(nn.Module):
    def __init__(self, resnet='resnet34', num_classes=2, pretrained=False, convert=True, newBN=AdaBN, n=5):
        super().__init__()
        if resnet == 'resnet34':
            base_model = resnet34
            bottleneck = False
        ...
        self.newBN = newBN
        if convert:
            self.res = convert_encoder_to_target(self.res, newBN, start=0, end=5, verbose=False, bottleneck=bottleneck,
                                                 n=n)
            self.up1, self.up2, self.up3, self.up4, self.bnout = convert_decoder_to_target(
                [self.up1, self.up2, self.up3, self.up4, self.bnout], newBN, start=0, end=5, verbose=False, n=n)
        ...
        for i, layer in enumerate(layers):
            if i == 0:
                ...
                for j, block in enumerate(layer):
                    self.feature_hooks.append(SaveFeatures(block.bn1, str(i) + '-bn1'))
                    self.feature_hooks.append(SaveFeatures(block.bn2, str(i) + '-bn2'))
                    if resnet == 'resnet50':
                        self.feature_hooks.append(SaveFeatures(block.bn3, str(i) + '-bn3'))

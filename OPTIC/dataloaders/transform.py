...
def get_weak_transform(patch_size=(512, 512)):
    tr_transforms = []
    tr_transforms.append(
        SpatialTransform_2(
            patch_size, [i // 2 for i in patch_size],
...
def get_strong_transform(patch_size=(512, 512)):
    tr_transforms = []
    tr_transforms.append(BrightnessMultiplicativeTransform((0.7, 1.5), per_channel=True, p_per_sample=0.15))
...
def collate_fn_color_transform(batch):
    ...
    data_dict = {'ori': ori_img, 'data': color_img, 'mask': label, 'name': name}
    return data_dict
...
def collate_fn_TTA_transform(batch):
    image, label, name = zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    ...
    tr_transforms = get_strong_transform()
    aug_imgs = [image.copy()]
    for time in range(16 - 1):
        aug_dict = tr_transforms(**data_dict)
        aug_imgs.append(aug_dict['data'])
        data_dict['data'] = aug_imgs[0].copy()

    data_dict['data'] = np.squeeze(np.stack(aug_imgs, 0), axis=1)
    data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    return data_dict

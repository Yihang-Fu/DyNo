...
class Res2Net(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, norm=nn.BatchNorm2d):
        downsample = None
        ...
        layers.append(block(self.inplanes, planes, stride, downsample=downsample,
                            stype='stage', baseWidth=self.baseWidth, scale=self.scale, norm=norm))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, baseWidth=self.baseWidth, scale=self.scale, norm=norm))
